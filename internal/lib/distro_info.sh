...
function to_testbox_image()
{
	...
	if [ -n "$VERSION" ]; then
		  echo ghcr.io/phusion/passenger_apt_automation_testbox_ubuntu_${VERSION/./_}:2.2.2
		  return
	fi

	VERSION=$(awk -F, -vPATTERN="^$INPUT(\\\\.0)?\$" "$AWK_SCRIPT" /usr/share/distro-info/debian.csv)
	if [ -n "$VERSION" ]; then
		  echo ghcr.io/phusion/passenger_apt_automation_testbox_debian_${VERSION/./_}:2.2.2
		  return
	fi
...
function is_ubuntu ()
{
    grep -q "$1" /usr/share/distro-info/ubuntu.csv
}

function is_debian ()
{
    grep -q "$1" /usr/share/distro-info/debian.csv
}
